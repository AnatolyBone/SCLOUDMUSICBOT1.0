...
async function trackDownloadProcessor(task) {
  try {
    ...
    try {
      ...
      const ytdlArgs = {
        output: tempFilePath, 'embed-thumbnail': true, 'add-metadata': true,
        'convert-thumbnails': 'jpg', ...YTDL_COMMON
      };
      if (canCopyMp3(ext, acodec)) {
        await ytdl(url, ytdlArgs);
      } else {
        await ytdl(url, { ...ytdlArgs, 'extract-audio': true, 'audio-format': 'mp3' });
      }
      ...
      const sentToUserMessage = await bot.telegram.sendAudio(userId,
      ...
      if (sentToUserMessage?.audio?.file_id) {
        ...
        if (STORAGE_CHANNEL_ID) {
          try {
            ...
            await db.cacheTrack({ url: cacheKey, fileId: sentToStorage.audio.file_id, title, artist: uploader, duration: roundedDuration, thumbnail });
  ...
  (async () => {
    ...
    try {
      ...
      let tracksToProcess = entries.filter(e => e && (e.webpage_url || e.url)).map(e => {
        ...
        const md = {
          id: e.id, title: sanitizeFilename(e.title || 'Unknown Title'), uploader: e.uploader || 'Unknown Artist',
          duration: e.duration, thumbnail: e.thumbnail, ext, acodec, filesize
        };
