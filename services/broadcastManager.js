...
/**
 * Выполняет отправку одной задачи рассылки на максимальной скорости.
...
export async function runSingleBroadcast(bot, task, users, taskId = null) {
    ...
    for (const user of usersToSend) {
        ...
        // Базовая задержка для защиты от API лимитов Telegram (~25 сообщений в секунду)
    ...
    if (isPreview) {
        ...
        if (users.length > 1 || (users.length === 1 && users[0].id !== ADMIN_ID)) {
           try {
                ...
                console.error('Не удалось отправить отчет админу:', e.message); 
            }
        }
    }
