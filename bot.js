...
const playlistSessions = new Map(); // Хранилище сессий для выбора треков
const TRACKS_PER_PAGE = 5; // Треков на странице выбора
...
bot.start(async (ctx) => {
    ...
    const user = await getUser(ctx.from.id); // Перезапросим пользователя, чтобы получить свежие данные
...
const trackTitle = title.length > 50 ? title.slice(0, 47) + '...' : title;
    ...
    const messageText = `🎶 <b>${title}</b>\n\nВыберите треки для скачивания (Стр. ${currentPage + 1}/${totalPages}):`;
    return {
        text: messageText,
...
bot.action(/pl_download_10:(.+)/, async (ctx) => {
    ...
    await ctx.editMessageText(`✅ Отлично! Проверяю лимиты и ставлю треки в очередь...`);
    
    const user = await getUser(userId);
    const remainingLimit = user.premium_limit - (user.downloads_today || 0);
    if (remainingLimit <= 0) return await ctx.reply(T('limitReached'));
    
    const tracksToProcess = session.tracks.slice(0, 10).slice(0, remainingLimit);
...
if (data.entries && data.entries.length > 0) {
    ...
    // ...
            ...
            // Напрямую добавляем в очередь, минуя старый enqueue
            downloadQueue.add({
                userId: ctx.from.id,
                source: 'soundcloud',
