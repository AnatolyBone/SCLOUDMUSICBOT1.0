...
export async function createUser(id, first_name = '', username = '', referral_source = null, referrer_id = null) {
  // =====> ДОБАВЬТЕ ЭТОТ "ЖУЧОК" <=====
  console.log(`[DEBUG] Checkpoint 4 (createUser): Received referrer_id = ${referrer_id}`);
...
export async function getUser(id, firstName = '', username = '', startPayload = null) {
  console.log(`[DEBUG] Checkpoint 2 (getUser): startPayload = ${startPayload}`);
  ...
  if (rows.length > 0) {
    ...
    console.log(`[DEBUG] Checkpoint 3 (getUser -> else): Parsed referrerId = ${referrerId}. Calling createUser...`);
