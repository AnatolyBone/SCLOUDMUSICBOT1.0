...
// === ГЛОБАЛЬНЫЙ MIDDLEWARE АВТОРИЗАЦИИ ===
const requireAuth = (req, res, next) => {
  if (req.session && req.session.authenticated && req.session.userId === ADMIN_ID) {
    return next();
  }
  res.redirect('/admin');
};
...
function setupExpress() {
  ...
  app.get('/debug/state', requireAuth, async (req, res) => {
    // Предположим, getAppState доступен или можно импортировать
    // Если getAppState нет в db.js или imports, можно вернуть упрощенный объект
    const state = {
...
app.get('/admin/user/:id/links', requireAuth, async (req, res) => {
  try {
    const userId = req.params.id;
    const urls = await getUserUniqueDownloadedUrls(userId); // Эта функция теперь импортирована
...
// 2. Починить кэш для конкретного пользователя (с датой)
app.post('/admin/user/:id/fix-cache', requireAuth, async (req, res) => {
  try {
    const userId = req.params.id;
    const { fixDate } = req.body; // Получаем дату из формы

    const count = await fixBadCacheForUser(userId, fixDate); // Эта функция теперь импортирована
